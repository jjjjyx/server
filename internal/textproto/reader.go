...
func (r *Reader) ReadMIMEHeader() (textproto.MIMEHeader, error) {
	header, _, err := readMIMEHeader(r, math.MaxInt64, math.MaxInt64)
	return header, err
}

func (r *Reader) ReadMIMERequestHeader() (textproto.MIMEHeader, []string, error) {
	return readMIMEHeader(r, math.MaxInt64, math.MaxInt64)
}

// readMIMEHeader is a version of ReadMIMEHeader which takes a limit on the header size.
// It is called by the mime/multipart package.
func readMIMEHeader(r *Reader, maxMemory, maxHeaders int64) (textproto.MIMEHeader, []string, error) {
	...
	if buf, err := r.R.Peek(1); err == nil && (buf[0] == ' ' || buf[0] == '\t') {
		...
		return m, nil, textproto.ProtocolError("malformed MIME header initial line: " + string(line))
	}
	// 一般请求头也就这么多了
	keysOriginOrder := make([]string, 15)

	for {
		kv, err := r.readContinuedLineSlice(maxMemory, mustHaveFieldNameColon)
		if len(kv) == 0 {
			return m, keysOriginOrder, err
		}

		// Key ends at first colon.
		k, v, ok := bytes.Cut(kv, colon)
		if !ok {
			return m, keysOriginOrder, textproto.ProtocolError("malformed MIME header line: " + string(kv))
		}

		if tk := string(k); tk != "" {
			keysOriginOrder = append(keysOriginOrder, tk)
		}
		...
		maxHeaders--
		if maxHeaders < 0 {
			return nil, keysOriginOrder, errMessageTooLarge
		}
		...
		if maxMemory < 0 {
			return m, keysOriginOrder, errMessageTooLarge
		}
